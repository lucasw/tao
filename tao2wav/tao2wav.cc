...
int main(int argc, char **argv) {
  ...
  // sample rate and number of channels and create the `filesetup' structure
  // which
  ...
  if (strcmp(fileid, "TAO OUTPUT FILE") != 0) {
    std::cerr << "Error: " << infilename
              << " is not a Tao output file, aborting\n";
    exit(1);
  }
  ...
  while (1) {
    ...
    for (sampleIndex = 0; sampleIndex < numSamplesRead; sampleIndex++) {
      shortSamples[sampleIndex] = (short)(floatSamples[sampleIndex] * scaleBy);
    }
