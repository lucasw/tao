...
void TaoCell::bow(float f_bow, float v_bow) {
  ...
  if (mode & TAO_CELL_BOW_STICK_MODE) // if in `stick' mode.
  {
    ...
    if (f_stick > f_bow)
      mode &= !TAO_CELL_BOW_STICK_MODE; // if static frictional
    else
      force_exerted = f_stick; // force required is too
  }                            // great, change to
                               // `slip' mode.
  ...
  else // if in `slip' mode.
  {
    ...
    if (velocity >= 0.0)
      mode |= TAO_CELL_BOW_STICK_MODE; // if the cell starts
    else
      force_exerted = f_slip; // travelling in the same
  }                           // (positive) direction
                              // `stick' mode.
