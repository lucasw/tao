...
// of discrete masses and springs arranged in a grid pattern, if x and y are
// real
...
#ifdef INSTRUMENT_DEBUG
  cout << "i=" << i << " j=" << j << "       ";
  cout << "X_=" << p.X_ << " Y_=" << p.Y_ << " X=" << p.X << " Y=" << p.Y
       << endl;
...
#ifdef INSTRUMENT_DEBUG
  std::cout << "defaultVelocityMultiplier=" << defaultVelocityMultiplier
            << std::endl;
#endif
...
#ifdef INSTRUMENT_DEBUG
std::cout << name << ".calculateForces()" << " rows=" << rows << " this=" <<
this << std::endl;
#endif
      ...
      if (c->mode & TAO_CELL_HAS_8_NEIGHBOURS) {
        c->force =
            (c->north->position + c->south->position + c->east->position +
             c->west->position + c->neast->position + c->seast->position +
             c->nwest->position + c->swest->position) -
            8.0 * c->position;
      }
...
//	new velocity by the velocityMultiplier value (between 0 and 1). This
//value is
...
TaoInstrument &TaoInstrument::setDamping(float position, float damping) {
  ...
  rows[0].cells[i].velocityMultiplier =
      1 - ((float)pow(10000.0f, damping) / 10000.0);
...
TaoInstrument &TaoInstrument::resetDamping(float x1, float x2, float y1,
  ...
  for (j = j1; j <= j2; j++) {
    ...
    for (i = i1; i <= i2; i++) {
      if (i >= imin && i <= imax) {
        rows[j].cells[i - imin].velocityMultiplier = defaultVelocityMultiplier;
      }
    }
  }
  return *this;
...
TaoInstrument &TaoInstrument::resetDamping(float left, float right) {
  ...
  for (i = i1; i <= i2; i++) {
    if (i >= imin && i <= imax) {
      rows[0].cells[i - imin].velocityMultiplier = defaultVelocityMultiplier;
    }
  }
  return *this;
}
...
void TaoInstrument::joinLeftToLeft(TaoCell &cell1, TaoCell &cell2) {
  ...
  if (c1)
    c1->swest = c1->south->west;
  if (c2)
    c2->swest = c2->south->west;
...
void TaoInstrument::joinBottomToBottom(TaoCell &cell1, TaoCell &cell2) {
  ...
  if (c1)
    c1->swest = c1->west->south;
  if (c2)
    c2->swest = c2->west->south;
