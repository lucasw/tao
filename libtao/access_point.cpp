...
float TaoAccessPoint::getVelocity() {
  ...
  case TAO____:
    ...
    break;
  case TAO___D:
    a = b = c = d = celld->velocity;
    break;
  case TAO__C_:
    a = b = c = d = cellc->velocity;
    break;
  case TAO__CD:
    a = c = cellc->velocity;
    b = d = celld->velocity;
...
float TaoAccessPoint::getPosition() {
  ...
  case TAO____:
    ...
    break;
  case TAO___D:
    a = b = c = d = celld->position;
    break;
  case TAO__C_:
    a = b = c = d = cellc->position;
    break;
  case TAO__CD:
    a = c = cellc->position;
    b = d = celld->position;
...
void TaoAccessPoint::applyForce(float f) {
  ...
  if (cellb)
    cellb->applyForce(f * X_ * Y);
  if (cellc)
    cellc->applyForce(f * X * Y_);
  if (celld)
    celld->applyForce(f * X_ * Y_);

  //    std::cout << "cellc=" << f*X*Y_ << " celld=" << f*X_*Y_ << std::endl;
  //    std::cout << "cella=" << f*X*Y  << " cellb=" << f*X_*Y << std::endl <<
  //    std::endl;
}
...
void TaoAccessPoint::connect(TaoAccessPoint &p1, TaoAccessPoint &p2,
  ...
  if (p1.cella) {
    if (p2.cella)
      faa = (p2.cella->position - p1.cella->position) * eaa;
    if (p2.cellb)
      fab = (p2.cellb->position - p1.cella->position) * eab;
    if (p2.cellc)
      fac = (p2.cellc->position - p1.cella->position) * eac;
    if (p2.celld)
      fad = (p2.celld->position - p1.cella->position) * ead;
  }

  if (p1.cellb) {
    if (p2.cella)
      fba = (p2.cella->position - p1.cellb->position) * eba;
    if (p2.cellb)
      fbb = (p2.cellb->position - p1.cellb->position) * ebb;
    if (p2.cellc)
      fbc = (p2.cellc->position - p1.cellb->position) * ebc;
    if (p2.celld)
      fbd = (p2.celld->position - p1.cellb->position) * ebd;
  }

  if (p1.cellc) {
    if (p2.cella)
      fca = (p2.cella->position - p1.cellc->position) * eca;
    if (p2.cellb)
      fcb = (p2.cellb->position - p1.cellc->position) * ecb;
    if (p2.cellc)
      fcc = (p2.cellc->position - p1.cellc->position) * ecc;
    if (p2.celld)
      fcd = (p2.celld->position - p1.cellc->position) * ecd;
  }

  if (p1.celld) {
    if (p2.cella)
      fda = (p2.cella->position - p1.celld->position) * eda;
    if (p2.cellb)
      fdb = (p2.cellb->position - p1.celld->position) * edb;
    if (p2.cellc)
      fdc = (p2.cellc->position - p1.celld->position) * edc;
    if (p2.celld)
      fdd = (p2.celld->position - p1.celld->position) * edd;
  }

  if (p1.cella)
    p1.cella->force += (faa + fab + fac + fad) * connectionStrength;
  if (p1.cellb)
    p1.cellb->force += (fba + fbb + fbc + fbd) * connectionStrength;
  if (p1.cellc)
    p1.cellc->force += (fca + fcb + fcc + fcd) * connectionStrength;
  if (p1.celld)
    p1.celld->force += (fda + fdb + fdc + fdd) * connectionStrength;

  if (p2.cella)
    p2.cella->force += (-faa - fba - fca - fda) * connectionStrength;
  if (p2.cellb)
    p2.cellb->force += (-fab - fbb - fcb - fdb) * connectionStrength;
  if (p2.cellc)
    p2.cellc->force += (-fac - fbc - fcc - fdc) * connectionStrength;
  if (p2.celld)
