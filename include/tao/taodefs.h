...
#define FromTo(a, b)                                                           \
  if (Tick <=                                                                  \
      ...
      Tick >=                                                                  \
          (long)((tao.newStart = (a)) * tao.synthesisEngine.modelSampleRate))
...
#define linear(y1, y2)                                                         \
  ((Time - tao.start) / (tao.end - tao.start) * (y2 - y1) + y1)
#define expon(y1, y2)                                                          \
  ((float)(y1)*exp(1.0 / (tao.end - tao.start) *                               \
                   log((float)(y2) / (float)(y1)) * (Time - tao.start)))

#define Param float

#define Print                                                                  \
  std::cout << std::setw(0) << std::setprecision(4)                            \
