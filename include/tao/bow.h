...
class DLLEXPORT Bow : public Device {
public:
  Bow(std::shared_ptr<Manager> tao);
  Bow(std::shared_ptr<Manager> manager, const std::string bowName);
  Bow &setForce(float force);
  Bow &setVelocity(float velocity);
  float getForce();
  float getVelocity();
